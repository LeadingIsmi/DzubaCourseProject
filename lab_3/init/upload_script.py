...
class Initializer():
    ...
    @staticmethod
    def init_data(users_count: int, count_of_products: int):
        print("Start initializing")
        Initializer.psql_init(users_count)
        Initializer.mongo_init(users_count=users_count,
    ...
    @staticmethod
    def mongo_init(users_count: int, count_of_products: int):
        print("Start initializing mongodb")
        products_collection = MongoConnector.get_collection(
            collection_name="products")
        carts_collection = MongoConnector.get_collection(
            collection_name="carts")
        if not carts_collection.find_one():
            products = Initializer.generate_products(
                count_of_products=count_of_products)
            inserted_products = products_collection.insert_many(
                products).inserted_ids
            print(inserted_products)
            carts = Initializer.generate_carts(
                users_count=users_count, products=inserted_products)
            carts_collection.insert_many(carts)
        else:
            print("Mongo collection already inited, skip initialization")
            return

        if carts_collection.find_one():
    ...
    @staticmethod
    def generate_products(count_of_products: int):
        faker = Faker()
        products = [
            {
                k: v for k, v in {
                    "name": faker.unique.word(),
                    "price": random.random() * 100}.items()
            }
    ...
    @staticmethod
    def generate_carts(users_count: int, products: list):
        carts = [
            {
                k: v for k, v in {
                    "user_id": i,
                    "products": random.choices(products, k=random.randint(0, 20))
                }.items()
            }
...
count_of_products: int = 100
users_count: int = 10000
Initializer().init_data(users_count=users_count,
                        count_of_products=count_of_products)
